...
void leecad (Tcad x){
	...
	Tatencion crearUno (void){
		...
		int elegirEspecialidad(void){
			int esp;
			do{
				printf("Ingrese especialidad de la atencion\n0)%s\n1)%s\n2)%s\n3)%s\n4)%s\n",especialidades[0],especialidades[1],especialidades[2],especialidades[3],especialidades[4]);
				scanf("%d",&esp);
			} while(esp < 0 || esp > 4);
			return esp;
		}
			void mostrarUno (Tatencion var){
				...
				printf("Especialidad de la atencion: %s \n--------------------\n",especialidades[var.especialidad]);
			}
				...
				long devolverDni (Tatencion x){
					...
					int compararEspecialidad (Tatencion x, int y){
						...
						void devolverEspecialidad (Tcad x, int i){
							strcpy(x, especialidades[i]);
						}
